...
void SetSh3(HANDLE output_handle, CONSOLE_SCREEN_BUFFER_INFO& CSBufInf, int INACTIVE_COLOUR, int ACTIVE_COLOUR, int& Sh4, int& Sh3, int& Sh2, int& Sh1, int & active, bool active_menu, int your_field[10][10])
{
	enum KeY
	{
		KEY_ARROW_DOWN = 80,
	...
	DrawV(CSBufInf, output_handle, position, 187, 3);
		...
		if (_kbhit())
		{
			...
			case KEY_ARROW_DOWN:
				...
				else
				{
					...
					DrawG(CSBufInf, output_handle, position, 187, 3);
				}
				break;
